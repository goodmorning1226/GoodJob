...
export default function ResumeBuilder({ experiences, initialTarget, embedded = false, startInEditor = false, onClose, onGenerated, onLibraryOpen, onSaved }: Props) {
  ...
  const resumeFiles = [{ id: "orbit-apm", title: "Associate Product Manager", company: "Orbit 數位產品", template: "ATS 專業版", score: 82, date: "今天 14:32", color: "green" }, { id: "daily-ux", title: "UX Research Assistant", company: "日日生活科技", template: "專案導向版", score: 76, date: "昨天", color: "purple" }, { id: "general", title: "通用求職履歷", company: "未指定職缺", template: "ATS 專業版", score: null, date: "8 月 22 日", color: "orange" }];
  ...
  if (mode === "library") return (
    <section className="resume-library page-enter">
      ...
      <div className="resume-file-grid">
        {filteredResumeFiles.map((resume) => <button className="resume-file" key={resume.title} onClick={() => { let links: Record<string, string[]> = {}; try { links = JSON.parse(window.localStorage.getItem("goodjob-resume-experience-links-v1") || "{}"); } catch { links = {}; } const linkedIndexes = experiences.map((experience, index) => links[resume.id]?.includes(experience.title) ? index : -1).filter((index) => index >= 0); setResumeExperienceIndexes([...new Set([0, 1, 2, ...linkedIndexes])]); setTemplate(resume.template === "專案導向版" ? "project" : "ats"); setTargetJob(resume.score ? `${resume.title} · ${resume.company}` : "通用履歷"); setMode("editor"); }}><div className={`resume-thumbnail ${resume.color}`}><div><strong>宋宇倫</strong><span /><span /><b /><b /><b /><i /></div></div><div className="resume-file-info"><span>{resume.company}</span><h3>{resume.title}</h3><p>{resume.template} · 1 頁</p><div><small>更新於 {resume.date}</small></div></div></button>)}
