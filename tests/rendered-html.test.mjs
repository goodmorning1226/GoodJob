...
test("server-renders the GoodJob role selection", async () => {
  ...
  assert.match(html, /今天想從哪一個角度/);
  assert.match(html, /我是使用者/);
  assert.match(html, /我是企業方/);
  assert.match(html, /Prototype 使用本機展示資料/);
...
test("keeps the published prototype free of paid API integration", async () => {
  const files = await Promise.all([
    ...
    "AudienceGate.tsx", "EnterprisePortal.tsx", "PublicProfile.tsx",
